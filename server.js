...
// Serve only public files (never wa_auth/, .env, server.log, deploy.sh)
const PUBLIC_FILES = [
  'index.html',
  'terms.html',
  '404.html',
...
app.use((req, res, next) => {
  const cleanPath = decodeURIComponent(req.path).replace(/^\/+/, '').split('?')[0];
  if (cleanPath === '' || cleanPath.startsWith('api/')) return next();
  const isSafe = PUBLIC_FILES.includes(cleanPath) || cleanPath.startsWith('assets/');
  if (!isSafe) return res.status(404).json({ error: 'Not found' });
  next();
});
